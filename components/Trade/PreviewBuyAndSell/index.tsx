...
const PreviewBuyAndSell = ({ type, onBack }: PreviewBuyAndSellProps) => {
  ...
  return confirm ? (
    ...
    <>
      ...
      <div>
        <Option classTitle="w-30" title="Pay with" color="bg-theme-green">
          USD Balance
        </Option>
        <Option classTitle="w-30" title="Price" color="bg-theme-green">
          1 ETH = US$3698,39
        </Option>
        <Option classTitle="w-30" title="NeutraNet fee" color="bg-theme-green">
          US$0.00
        </Option>
        <Option classTitle="w-30" title={type === 'buy' ? 'Buy' : 'Sell'}>
          Ethereum <span className="ml-2 text-theme-secondary">ETH</span>
        </Option>
